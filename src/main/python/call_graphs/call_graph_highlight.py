...
def found_the_node(project_path, f_name):
...
def add_highlight_to_data(project_path, solution_node):
    graph_file_path = project_path + "/static_call_graph.html"
...
def call_functions(project_path, node):
    node_id = found_the_node(project_path, node)
    solution = lighter(node_id)
    add_highlight_to_data(project_path, solution)


# args
project_path_args = sys.argv[1]
node_arg = sys.argv[2]

c_g_file_path = project_path_args + "/static_call_graph.html"
call_graph_file_path = Path(c_g_file_path)
if call_graph_file_path.is_file():
    call_functions(project_path_args, node_arg)
else:
    problem_with_the_file = open(project_path_args+"/static_call_graph_highlighted.html", 'w')
